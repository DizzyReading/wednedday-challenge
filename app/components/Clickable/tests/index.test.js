...
describe('<Clickable /> component tests', () => {
  ...
  it('should contain render the text according to the textId', () => {
    const { getAllByText } = renderWithIntl(<Clickable textId="details_list" />);
    expect(getAllByText(/Details List/).length).toBe(1);
  });

  it('should call the prop onClick when the clickable component is clicked', () => {
    const clickSpy = jest.fn();
    const { getAllByText, queryByText } = renderWithIntl(<Clickable onClick={clickSpy} textId="details_list" />);
    expect(getAllByText(/Details List/).length).toBe(1);
    fireEvent.click(queryByText(/Details List/));
